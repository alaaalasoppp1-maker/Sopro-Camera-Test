...
public class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b) {
        ...
        append("Sopro Camera Player v0.5.2");
    ...
    private void startCamera(boolean forceRetryProfiles) {
        ...
        new Thread(() -> {
            ...
            profiles.add(new VideoProfile("SAA7113 PAL 720x576", "SAA7113", 720, 576));
            profiles.add(new VideoProfile("SAA7113 NTSC 720x480", "SAA7113", 720, 480));
            profiles.add(new VideoProfile("Generic PAL 720x576", "GENERIC_ANALOG", 720, 576));
            profiles.add(new VideoProfile("Generic NTSC 720x480", "GENERIC_ANALOG", 720, 480));
            ...
            for (VideoProfile vp: profiles) {
                ...
                while (System.currentTimeMillis()<deadline && useful<8) {
                    byte[] block = nativeReadIso(fd, 0x82, selectedIsoPacketSize, 48, 700);
    ...
    private void beginLiveLoop() {
        ...
        streamThread = new Thread(() -> {
            int fd = conn.getFileDescriptor();
            while (streaming.get()) {
                byte[] block = nativeReadIso(fd, 0x82, selectedIsoPacketSize, 64, 800);
    ...
    private class FrameAssembler {
        ...
        private void finishCurrentField(boolean render) {
            ...
            boolean plausible = fieldBytes > expected / 2;
            if (plausible) {
                if (currentTop) topReady = true;
                else bottomReady = true;
            }

            if (render && topReady && bottomReady) {
                renderFrame();
                topReady = false;
                bottomReady = false;
            }
        ...
        private void renderFrame() {
            ...
            for (int i = 0; i + 3 < yuyv.length && out + 1 < argb.length; i += 4) {
                int y0 = yuyv[i] & 0xff;
                int u  = yuyv[i + 1] & 0xff;
